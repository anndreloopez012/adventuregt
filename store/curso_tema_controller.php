...
$arrContent = array();
$intId = $_GET["id"];
$var_consulta= "SELECT * FROM content_s WHERE id = '$intId' LIMIT 1 
";

$qTMP = mysqli_query($link, $var_consulta);
while ( $rTMP = mysqli_fetch_assoc($qTMP) ){
    $arrContent[$rTMP["id"]]["id"] = $rTMP["id"];
    $arrContent[$rTMP["id"]]["subtitulo"] = $rTMP["subtitulo"];
    $arrContent[$rTMP["id"]]["requisitos"] = $rTMP["requisitos"];
    $arrContent[$rTMP["id"]]["contenido"] = $rTMP["contenido"];
    $arrContent[$rTMP["id"]]["video"] = $rTMP["video"];
...
WHERE ct.id_producto=id_content_t
";
